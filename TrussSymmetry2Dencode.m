function [a,b] = TrussSymmetry2Dencode
...
xnum=SymmetryIndex;
SymmetryBoundLeft = lineX(SymmetryIndex);
SymmetryBoundRight = lineX(SymmetryIndex+1);
width = PRB.dv.xMax - PRB.dv.xMin;
width1p = width*0.0001;
...
%Analysis Couple FixNode (For Symmetry Analysis)
global SymmetryNodeFix;
SymmetryNodeFix=zeros(NOF.FixNode,1);
for i=1:NOF.FixNode
    if PRB.bc.node(i,1) < SymmetryBoundLeft
        for j=i:NOF.FixNode
            if PRB.bc.node(j,1)>SymmetryBoundRight
                NodeFromRight = [(width-PRB.bc.node(j,1)) PRB.bc.node(j,2)];
                tLength = norm([(PRB.bc.node(i,1)-NodeFromRight(1)) (PRB.bc.node(i,2)-NodeFromRight(2))]);
                if tLength<width1p
                    SymmetryNodeFix(i,1)=j;
                end
            end
        end
    end
